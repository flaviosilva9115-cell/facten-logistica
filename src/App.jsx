...
// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function Dashboard({pedidos,tarefas,users,obras,fornecedores,onOpenPedido}){
  ...
  return<div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10,marginBottom:20}}>
      {cards.map(c=><div key={c.l} style={{background:c.bg,borderRadius:12,padding:"14px 14px",border:"1px solid "+c.c+"25"}}>
    ...
    {atrasados.length>0&&<div style={{background:"#FBE9E7",border:"1px solid "+G.orange+"40",borderRadius:12,padding:"12px 16px",marginBottom:16}}>
      <div style={{fontWeight:700,fontSize:13,color:G.orange,marginBottom:8}}>⚠️ {atrasados.length} pedido(s) com entrega atrasada — clique para ver</div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
      <div style={{background:G.surface,borderRadius:14,padding:18,border:"1px solid "+G.border}}>
        <div style={{fontSize:13,fontWeight:800,marginBottom:14}}>Pedidos por Status</div>
        <ResponsiveContainer width="100%" height={180}><PieChart><Pie data={byStat} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({name,value})=>value>0?`${name}(${value})`:""} labelLine={false} fontSize={10}>{byStat.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer>
      </div>
      <div style={{background:G.surface,borderRadius:14,padding:18,border:"1px solid "+G.border}}>
        <div style={{fontSize:13,fontWeight:800,marginBottom:14}}>Atividade — 7 Dias</div>
        <ResponsiveContainer width="100%" height={180}><LineChart data={week}><CartesianGrid strokeDasharray="3 3" stroke={G.border}/><XAxis dataKey="dia" tick={{fontSize:10}} stroke={G.light}/><YAxis tick={{fontSize:10}} stroke={G.light}/><Tooltip/><Legend wrapperStyle={{fontSize:10}}/><Line type="monotone" dataKey="Criados" stroke={G.green} strokeWidth={2} dot={{r:3}}/><Line type="monotone" dataKey="Entregues" stroke={G.blue} strokeWidth={2} dot={{r:3}}/></LineChart></ResponsiveContainer>
      </div>
    </div>
    <div style={{background:G.surface,borderRadius:14,padding:18,border:"1px solid "+G.border}}>
      <div style={{fontSize:13,fontWeight:800,marginBottom:14}}>Pedidos por Obra</div>
      <ResponsiveContainer width="100%" height={180}><BarChart data={byObra} barSize={14}><CartesianGrid strokeDasharray="3 3" stroke={G.border}/><XAxis dataKey="name" tick={{fontSize:10}} stroke={G.light}/><YAxis tick={{fontSize:10}} stroke={G.light}/><Tooltip/><Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="E" name="Entregues" fill={G.green} radius={[3,3,0,0]}/><Bar dataKey="P" name="Pendentes" fill={G.gold} radius={[3,3,0,0]}/></BarChart></ResponsiveContainer>
    </div>
  </div>;
}
    ...
    <div style={{display:"flex",minHeight:"100vh",fontFamily:"Inter,sans-serif",color:G.text,background:G.bg}}>
      ...
      <div style={{flex:1,overflowY:"auto",minWidth:0}}>
        ...
        <div style={{padding:24}}>
          {page==="dashboard"&&<Dashboard pedidos={pedidos} tarefas={tarefas} users={users} obras={obras} fornecedores={fornecedores} onOpenPedido={p=>{setDetailP(p);setPage("pedidos");}}/>}
